...
extern "C" __declspec(dllimport) BSTR DllMakeCublasHandle(void **dev_handle);

extern "C" __declspec(dllimport) BSTR DllDestroyCublasHandle(void *dev_handle);

//C = α op ( A ) op ( B ) + β C
extern "C" __declspec(dllimport) BSTR DllcublasSgemm(
	...
	int ldc);           // leading dimension of a two-dimensional array used to store the matrix C. 
                           //... (ie. the number of rows in a matrix stored in column major format)